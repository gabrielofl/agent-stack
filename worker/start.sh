...
/app/model-server -m "$MODEL_PATH" --host 0.0.0.0 --port 8080 --ctx-size "$CTX_SIZE" &
MODEL_PID=$!

# If model-server dies immediately, don't continue
sleep 1
if ! kill -0 "$MODEL_PID" 2>/dev/null; then
  echo "model-server crashed immediately. Exiting."
  exit 1
fi
...
if [ "$READY" -ne 1 ]; then
  echo "model-server did not become ready in time. Exiting."
  exit 1
fi
